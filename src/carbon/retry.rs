...
pub async fn queue_start_relay(carbon_config: &Carbon, pool: Arc<PgPool>, carbon_broadcaster: Sender<BroadcastRequest>, nonce: BigDecimal) {
...
async fn expire_pending_actions(carbon_config: &Carbon, pool: Arc<PgPool>, carbon_broadcaster: Sender<BroadcastRequest>) -> Result<()> {
    ...
    let expired_nonces: Vec<u64> = expired_events
        .into_iter()
...
pub async fn prune_processed_nonces(rest_url: &str, pool: Arc<PgPool>, expired_nonces: &[u64]) -> Result<Vec<u64>> {
    ...
    let nonces_to_delete: Vec<u64> = expired_nonces
        .iter()
        .cloned()
    ...
    let nonces_to_process: Vec<u64> = expired_nonces
        .iter()
        .cloned()
        .filter(|nonce| pending_nonces.contains(nonce))
        .collect();
